...
* One issue that I have only noticed on a wide screen 21:9, was that the image on the contact-us page was not maintaining its aspect ratio. It was stretching, from heading to the footer, ocupying the white margings that I had set. This problem was solved after changing the display propetry of the div containing the image and the form, from inline to flex and aligned them to the center of the page. That displayed the image and the form next to each other. I then added flex:1 property and a max-width of 47% for both columns. In this way I managed to make the image and the form grow and shrink symmetrically and proportional on each screen width, including the wide screen, mainteining their aspect-ratio.

* When I added the hover over effect to the grid gallery items (which contain 2 divs: the image and the description box), namely the orange border, I have noticed that the images would zoom out a bit and overlap the border and some of them would have a double border. After checking with *Devtools* activating and deactivating various properties, I have realized that the problem was caused by the property *border: 4px solid #ff852e* that I set for the img when hovered over. As the images already had a border and I only wanted to add the change of color for the border, this property was adding an extra border. So I have replaced that property with *border-color: #ff852e* and it worked.

* I wanted to increase the  83 *Lighthouse* *Best Practices* score for the *Adopt* page. One of the suggestions that it gave me for that was to add width and height properties to all the img elements from that page, including the logo. I did so for the logo first to try it and the score increased indeed, but on small screens 750px wide and below, the header started to behave very strange, moving on top of the logo, together with the burger button amd decreasing its height. I then have removed the width and height properties and everything was normal again.
